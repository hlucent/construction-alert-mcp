...
## 상태
✅ 누구나 사용 가능한 공개 MCP 서버 — 배포 주소: `<배포 후 업데이트 예정>`
...
### 커넥터 연결 방법
이 서버는 누구나 접근 가능한 공개 MCP 서버다. 별도의 인증 키 없이 아래 주소로 바로 연결하면 된다.
```
<배포 후 업데이트 예정>/mcp
```
서울 열린데이터광장 호출용 키(`SEOUL_OPENAPI_KEY`)는 서버가 업스트림 API를 호출할 때만 쓰이며, 서버가 자체적으로 보유한 값이라 사용자가 URL에 붙이거나 별도로 입력할 필요가 없다.
